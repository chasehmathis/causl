...
vignette: >
  %\VignetteIndexEntry{Copula Specification}
...
cmr <- modify(cmr, formulas=forms2, method="rejection",
...
```{r mod_pc, eval=TRUE}
cmp <- cmr
cmp <- modify(cmp, method = "inversion",
                          pars = list(cop=list(
...
change very easily.
```{r fam_pc}
cmp2 <- modify(cmp, family = list(c(1,1), 5, 1, c(3,1)))  # use Clayton+Gaussian copulas
